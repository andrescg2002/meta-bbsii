...
if [ ${PART_SCRIPT} == "/usr/bin/emmc_mk4parts.sh" ]; then
	echo "Formatting partition /dev/mmcblk1p4 as ext4"
	mkfs.ext4 -q -F /dev/mmcblk1p4

	if [ $? -ne 0 ]; then
		echo "Failed formatting /dev/mmcblk1p4"
		exit 1
	fi

	/usr/bin/emmc_copy_data.sh

	if [ $? -ne 0 ]; then
		echo "Failed copying /dev/mmcblk1p4"
		exit 1
	fi
